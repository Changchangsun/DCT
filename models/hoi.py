...
class CDNHOI(nn.Module):
    # backbone, cdn, num_obj_classes = args.num_obj_classes, num_verb_classes = args.num_verb_classes, num_queries = args.num_queries, aux_loss = args.aux_loss, args = args
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False,
        ...
        self.verb_obj = nn.Linear(hidden_dim + 8, hidden_dim + 8)
        self.obj = nn.Linear(num_obj_classes + 1, hidden_dim + 8)

        self.obj2verb = nn.Linear(num_obj_classes + 1, hidden_dim)
        self.action = nn.Linear(hidden_dim, hidden_dim)

        self.obj_verb = nn.Linear(hidden_dim, hidden_dim)
        self.verb_obj = nn.Linear(hidden_dim, hidden_dim)

        self.obj_verb_a = nn.Linear(hidden_dim, hidden_dim)
        self.verb_obj_a = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, samples: NestedTensor):
        ...
        hopd_out_h, hopd_out_o, object_decoder_out, interaction_decoder_out = self.transformer(self.input_proj(src),
                                                                                               mask,
                                                                                               self.query_embed_h.weight,
                                                                                               self.query_embed_o.weight,
                                                                                               self.query_embed_object.weight,
                                                                                               pos[-1])[:4]
        outputs_sub_coord = self.sub_bbox_embed(hopd_out_h).sigmoid()
        outputs_obj_coord = self.obj_bbox_embed(hopd_out_o).sigmoid()

        if self.use_matching:  # default not use
            outputs_matching = self.matching_embed(hopd_out)

        src_logits_obj = self.obj_verb(interaction_decoder_out)
        object_decoder_out1 = object_decoder_out + torch.mul(
            self.verb_obj_a(object_decoder_out).sigmoid(), src_logits_obj)
        outputs_obj_class = self.obj_class_embed(object_decoder_out1)

        src_logits_verb = self.verb_obj(object_decoder_out)
        interaction_decoder_out1 = interaction_decoder_out + torch.mul(
            self.obj_verb_a(interaction_decoder_out).sigmoid(), src_logits_verb)
        outputs_verb_class = self.verb_class_embed(interaction_decoder_out1)
...
#
#     def forward(self, x):
#         for i, layer in enumerate(self.layers): #3=(0,1,2)
#             if i == 1:
#                 x_pre = layer(x)
#             x = F.relu(layer(x)) if i < self.num_layers - 1 else layer(x)
#         return x,x_pre
