...
class CDN(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.object_decoder = TransformerDecoder(object_decoder_layer, num_dec_layers_interaction,
                                                      object_decoder_norm,
                                                      return_intermediate=return_intermediate_dec)
        ...
        self.nhead = nhead
        self.red = nn.Linear(3*d_model, d_model)
        self.mlp = MLP(3*d_model, d_model, d_model, 3)
        # self.red1 = nn.Linear(2*d_model, d_model)
        # self.red2 = nn.Linear(2*d_model, d_model)
    ...
    def forward(self, src, mask, query_embed_h,query_embed_o,query_embed_object,pos_embed):
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
        query_embed_h = query_embed_h.unsqueeze(1).repeat(1, bs, 1)
        query_embed_o = query_embed_o.unsqueeze(1).repeat(1, bs, 1)
        query_embed_object = query_embed_object.unsqueeze(1).repeat(1, bs, 1)

        mask = mask.flatten(1)
        tgt_h = torch.zeros_like(query_embed_h)
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)

        hopd_out_h = self.decoder(tgt_h, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed_h)
        hopd_out_h = hopd_out_h.transpose(1, 2)

        tgt_o = torch.zeros_like(query_embed_o)
        hopd_out_o = self.decoder(tgt_o, memory, memory_key_padding_mask=mask,
                                pos=pos_embed, query_pos=query_embed_o)
        hopd_out_o = hopd_out_o.transpose(1, 2)

        tgt_object = torch.zeros_like(query_embed_object)
        hopd_out_object = self.object_decoder(tgt_object, memory, memory_key_padding_mask=mask,
                                  pos=pos_embed, query_pos=query_embed_object)
        hopd_out_object = hopd_out_object.transpose(1, 2)
        hopd_out = self.mlp(torch.cat((hopd_out_h, hopd_out_o, hopd_out_object),dim = 3))
        ...
        return hopd_out_h,hopd_out_o,hopd_out_object, interaction_decoder_out, memory.permute(1, 2, 0).view(bs, c, h, w)
